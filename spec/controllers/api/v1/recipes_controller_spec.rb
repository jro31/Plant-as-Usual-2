  ...
  describe 'GET #show' do
    ...
    context 'recipe exists' do
      ...
      context 'it has many ingredients' do
        ...
        it { expect(JSON.parse(response.body)['ingredients'][0]['recipe_id']).to eq(recipe.id) }
        it { expect(JSON.parse(response.body)['ingredients'][0]['amount']).to eq('Some').or(eq('100')).or(eq(nil)) }
        it { expect(JSON.parse(response.body)['ingredients'][0]['unit']).to eq(nil).or(eq('grams')) }
        it { expect(JSON.parse(response.body)['ingredients'][0]['food']).to eq('Green leaves').or(eq('Bountiful fruit')).or(eq('Fanciful loot')) }
        it { expect(JSON.parse(response.body)['ingredients'][0]['preparation']).to eq('Shredded').or(eq(nil)) }
        ...
        it { expect(JSON.parse(response.body)['ingredients'][1]['amount']).to eq('Some').or(eq('100')).or(eq(nil)) }
        it { expect(JSON.parse(response.body)['ingredients'][1]['unit']).to eq(nil).or(eq('grams')) }
        it { expect(JSON.parse(response.body)['ingredients'][1]['food']).to eq('Green leaves').or(eq('Bountiful fruit')).or(eq('Fanciful loot')) }
        it { expect(JSON.parse(response.body)['ingredients'][1]['preparation']).to eq('Shredded').or(eq(nil)) }
        ...
        it { expect(JSON.parse(response.body)['ingredients'][2]['amount']).to eq('Some').or(eq('100')).or(eq(nil)) }
        it { expect(JSON.parse(response.body)['ingredients'][2]['unit']).to eq(nil).or(eq('grams')) }
        it { expect(JSON.parse(response.body)['ingredients'][2]['food']).to eq('Green leaves').or(eq('Bountiful fruit')).or(eq('Fanciful loot')) }
        it { expect(JSON.parse(response.body)['ingredients'][2]['preparation']).to eq('Shredded').or(eq(nil)) }
